...
export default function Navbar() {
  ...
  const menu = (
    <>
      ...
      <li>
        <Link onClick={() => window.scrollTo(0, 0)} to="/menu">
          Menu
        </Link>
      </li>
      <li>
  ...
  return (
    <div className="navbar bg-[#f6f7fb] dark:bg-gray-900 sticky w-full top-0 z-50 font-nunito font-light 2xl:container px-5 m-auto border-b-2 border-[#6c839b] dark:border-orange-600 text-black dark:text-current shadow-xl">
        ...
        <div>
          <Link to="/" className="btn btn-ghost normal-case">
            <div className="flex flex-col">
              <span className="text-4xl tracking-widest font-vibes text-[#c8a97e] relative">
                Cashless
              </span>
              <span className="tracking-widest font-nunito text-[#6c839b] dark:text-white absolute top-1/2 left-[120px] lg:left-16 text-base font-black inline-block">
                Restaurant
              </span>
            </div>
          </Link>
      ...
      <div className="navbar-end">
        {user ? (
          <div className="dropdown dropdown-end">
            ...
            >
              <div className="py-3 px-4 text-sm text-black dark:text-white">
                <div className="font-nunito font-semibold antialiased">
                  {user?.displayName}
                </div>
              ...
              <li>
                <Link
                  to="/dashboard"
                  className="flex items-center py-2 px-4 hover:bg-gray-200 dark:hover:bg-gray-600 dark:hover:text-white"
                >
                  <svg
                    aria-hidden="true"
          ...
          <Link
            to="/login"
            className="text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800 font-medium rounded-lg text-sm px-5 py-2 text-center mr-2 mb-0"
          >
            Login
          </Link>
        )}
